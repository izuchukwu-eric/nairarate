...
import { DAILY_OUTLIER_RATIO, rejectOutliers } from '../src/compute/sanity'
...
async function main(): Promise<void> {
  ...
  // Truncates. Any rows from a previous run are discarded, and re-fetching them
  // costs again — so apply the SQL before re-running.
  await writeFile(
    outPath,
  ...
  for (const target of targets) {
    let page = 1
    let fetchedForTicker = 0

    // Page size is FIXED for the whole ticker. It must not shrink as the budget
    // tightens: the API derives its offset from (page - 1) * limit, so a changed
    // limit mid-walk silently re-requests rows already paid for — page 1 at limit
    // 100 covers rows 1-100, then page 2 at limit 50 covers rows 51-100 again, and
    // per-row billing charges for them twice. Instead, stop the ticker when a full
    // page is no longer affordable and leave the remainder unspent.
    const limit = opts.limit

    // Hard iteration cap: a wrong `total` upstream must not become an unbounded
    // paid loop. The deepest pair is ~1,051 candles, so 50 pages of 100 is ample.
    const MAX_PAGES = 50
    ...
    for (; page <= MAX_PAGES; page++) {
      ...
      if (affordable < limit) {
        console.warn(
          `\n  BUDGET STOP: NGN ${spentNgn} of ${opts.budgetNgn} spent; ` +
            `remaining cannot cover a full ${limit}-row page. Stopping at ${target.ticker} ` +
            `page ${page}. Everything already fetched is in ${opts.out}.`,
        )
        break
      }
      ...
      // Screen before writing, exactly as the CBN backfill does. Monierate's own
      // history carries bad candles too — the first run found USD parallel at
      // 7954.60 on 2025-10-01 against a ~1484 local median, which would otherwise
      // land in the `high` field of a paid /history response on a product whose
      // entire claim is rate accuracy. USDT and USDC carried bad rows on the same
      // date at 3.17x, hence DAILY_OUTLIER_RATIO rather than the looser CBN ratio.
      const points = entries
        .filter((c): c is Candle & { date: string } => typeof c.date === 'string')
        .map((c) => ({
          currency: target.currency,
          rateDate: c.date,
          mid: c.avg_composite_rate ?? c.close ?? 0,
          candle: c,
        }))
      const screened = rejectOutliers(points, DAILY_OUTLIER_RATIO)
      for (const { row, localMedian, ratio } of screened.rejected) {
        console.warn(
          `    dropped ${row.currency} ${row.rateDate}: mid=${row.mid} vs local median ` +
            `${localMedian.toFixed(4)} (${ratio.toFixed(2)}x > ${DAILY_OUTLIER_RATIO}x)`,
        )
      }
      rejectedRows += screened.rejected.length

      const tuples = screened.kept
        .map((pt) => toValuesTuple(pt.candle, target.currency, target.market, updatedAt))
      ...
      fetchedForTicker += entries.length
      const total = body.data?.total ?? 0
      if (fetchedForTicker >= total || entries.length < limit) break
    }

    if (page > MAX_PAGES) {
      console.warn(`  ${target.ticker}: hit the ${MAX_PAGES}-page cap; series may be incomplete`)
    }
    // Out of budget for a full page — no point starting the next ticker.
    if (Math.floor((opts.budgetNgn - spentNgn) / RATE_QUOTE_COST_NGN) < opts.limit) break
  }

  console.log(`\n  Wrote ${rowsWritten} row(s) to ${opts.out}`)
  if (rejectedRows > 0) {
    console.log(`  Rejected ${rejectedRows} implausible candle(s) — see the warnings above`)
  }
